# OpenClaw Production Upgrade - Complete Decision Matrix
...
// Lines 408, 411, 414, 432
"skills": {
  "entries": {
    "nano-banana-pro": { "apiKey": "sk-proj-..." },
    "notion": { "apiKey": "ntn_..." },
    "sag": { "apiKey": "sk_..." }
  }
}

// LaunchAgent plist lines 29-32
<key>BRAVE_API_KEY</key>
<string>BSAbmRO-...</string>
```

**Total exposed secrets**: **9 API keys** in plaintext
...
1. **GitHub Issue #9627**: Config write operations permanently resolve `${VAR}` to plaintext
   ...
   - Config backups contain exposed secrets
...
|--------|--------|-------|-------------|
| **Secrets in config** | 9 keys | 0 keys | ✅ 100% reduction |
...
| **Secrets in plist** | 3 keys | 0 keys | ✅ 100% reduction |
...
**gateway.err.log**: 7.6MB and growing
...
| Metric | Before | After | Improvement |
|--------|--------|-------|-------------|
| **Current log size** | 8.5MB | 8.5MB (then rotates) | - |
| **Max log size** | Unbounded | 30MB | ✅ Capped |
...
```bash
$ ls ~/.openclaw/backups/
openclaw.json.pre-discord-cleanup  # Manual backup, 6 days old
```

**Only 1 manual backup exists**
...
```bash
$ ls -ld ~/.openclaw
drwxr-xr-x  ~/.openclaw  # 755 - world readable!

$ ls -la ~/.openclaw/.env
-rw-------  ~/.openclaw/.env  # 600 - correct ✓
```

**Directory is world-readable**: Other users can browse
**Config files are world-readable**: Other users can read secrets
...
$ ls /Users/YOUR_USER/.openclaw/
openclaw.json  logs/  agents/  # Can see everything!

$ cat /Users/YOUR_USER/.openclaw/openclaw.json
...
- No alerting
- **You don't know when bot is down**
...
| **Reliability** | 4/10 ⚠️ | 9/10 ✅ | +5 | HIGH |
| **Monitoring** | 2/10 🚨 | 8/10 ✅ | +6 | MEDIUM |
| **Operations** | 3/10 ⚠️ | 9/10 ✅ | +6 | MEDIUM |
| **OVERALL** | **3/10** | **9/10** | **+6** | **CRITICAL** |
...
| Scenario | Before Recovery | After Recovery | Improvement |
|----------|-----------------|----------------|-------------|
...
**Security Posture**: 2/10 → 9/10
**Operational Excellence**: 3/10 → 9/10
