...
OUTPUT_FILE="$WORKSPACE/.learnings/extracted-$(date +%Y-%m-%d).md"
...
for file in "$WORKSPACE"/memory/2026-02-*.md; do
    if [ -f "$file" ]; then
...
for pattern in "wrong" "actually" "mistake" "correction" "should be"; do
    COUNT=$(grep -ri "$pattern" "$WORKSPACE/memory/2026-02-"*.md 2>/dev/null | wc -l | tr -d ' ')
