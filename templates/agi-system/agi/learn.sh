...
TODAY=$(date +%Y-%m-%d)
OUTPUT="$WORKSPACE/.learnings/analysis-$TODAY.md"
...
if [ -f "$WORKSPACE/.learnings/ERRORS.md" ]; then
    ...
    grep "^### " "$WORKSPACE/.learnings/ERRORS.md" | sort | uniq -c | sort -rn >> "$OUTPUT"
fi
echo "" >> "$OUTPUT"
...
if [ -f "$WORKSPACE/.learnings/ERRORS.md" ]; then
    ...
    grep "Lesson:" "$WORKSPACE/.learnings/ERRORS.md" | sort | uniq -d >> "$OUTPUT" || echo "None found" >> "$OUTPUT"
fi
echo "" >> "$OUTPUT"

# 3. Extract correction keywords from recent memory
echo "## Recent Correction Keywords" >> "$OUTPUT"
for file in "$WORKSPACE"/memory/2026-02-0*.md; do
    if [ -f "$file" ]; then
...
grep -h "Lesson:" "$WORKSPACE/.learnings/ERRORS.md" 2>/dev/null | tail -5 >> "$OUTPUT" || echo "None" >> "$OUTPUT"
